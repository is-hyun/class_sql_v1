...
-- 2. 가격이 1,000,000원 이상인 상품삭제
SELECT * FROM product WHERE price >= 1000000;
DELETE FROM product WHERE price >= 1000000;

-- 3. 상품 설명에 'Apple'이 포함되 상품 삭제
SELECT * FROM product WHERE description LIKE '%Apple%';
DELETE FROM product WHERE description LIKE '%Apple%';


-- 복합 조건 삭제
-- 1. 가격이 500,000원 이하이고 상품 설명에 '스마트'가 포함된 상품 삭제
SELECT * FROM product WHERE price <= 500000 AND description LIKE '%스마트%';
DELETE FROM product WHERE price <= 500000 AND description LIKE '%스마트%';

-- 2. 상품명이 '갤럭시'로 시작하고 가격이 1,000,000원 이상인 상품 삭제
SELECT * FROM product WHERE name LIKE '갤럭시%' AND price >= 1000000;
DELETE FROM product WHERE name LIKE '갤럭시%' AND price >= 1000000;


-- 도전과제
-- ID가 5 ~ 8인 상품 삭제
SELECT * FROM product WHERE id BETWEEN 5 AND 8;
DELETE FROM product WHERE id BETWEEN 5 AND 8;

-- 상품 설명에 '태블릿'이 포함된 상품 삭제
SELECT *FROM product WHERE description LIKE '%태블릿%';
DELETE FROM product WHERE description LIKE '%태블릿%';

-- 가격이 300,000원 이하인 상품 삭제
SELECT * FROM product WHERE price <= 300000;
DELETE FROM product WHERE price <= 300000;

-- 상품명이 '아이폰' 또는 '아이패드'로 시작하는 상품 삭제
SELECT * FROM product WHERE name LIKE '아이폰%' OR name LIKE '아이패드%';
DELETE FROM product WHERE name LIKE '아이폰%' OR name LIKE '아이패드%';
